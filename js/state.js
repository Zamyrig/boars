...
// ── Прогресс (из state.user, данные в БД) ────────────────────

export function isBossDefeated(bossId) {
  return (state.user?.defeated_bosses || []).includes(bossId);
}

export function isFarmOwned() {
  return !!state.user?.farm_owned;
}

export function defeatBoss(bossId) {
  if (!state.user) return;
  if (!Array.isArray(state.user.defeated_bosses)) state.user.defeated_bosses = [];
  if (!state.user.defeated_bosses.includes(bossId)) {
    state.user.defeated_bosses.push(bossId);
    // Сохраняем на сервер в фоне
    import('./api.js').then(({ apiFetch }) => {
      apiFetch('/api/progress/boss', {
        method: 'POST',
        body: JSON.stringify({ tg_id: state.user.tg_id, boss_id: bossId }),
      }).catch(e => console.warn('defeatBoss sync error', e));
    });
  }
}
...
export function buyFarm() {
  if (!state.user) return;
  state.user.farm_owned = true;
  // Сохраняем на сервер в фоне
  import('./api.js').then(({ apiFetch }) => {
    apiFetch('/api/progress/farm', {
      method: 'POST',
      body: JSON.stringify({ tg_id: state.user.tg_id }),
    }).catch(e => console.warn('buyFarm sync error', e));
  });
}
